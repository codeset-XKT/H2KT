...
class Logger:
    def __init__(self):
        self.writer = open("./log.txt", 'w', encoding='utf-8')

    def count(self, say, df):
        log = "%s, record num: %d, student num：%d, question num：%d, skill num: %d" % (
            say, len(df), len(df['user_id'].unique()), len(df['exercise'].unique()), len(df['topic'].unique()))
...
def get_quesDiff(train_df, question_id_dict):
    print(train_df['exercise'].value_counts().values)
    ques_few_cnt, quesID2diffValue_dict = 0, dict()
    for ques in question_id_dict.keys():
        tmp_df = train_df[train_df['exercise'] == ques]
        if len(tmp_df) >= 3:
...
def get_train_test_area(train_or_test):
    ques_tmp_df = pd.read_csv("graph/ques_area.csv")
    quesID2tmpID_dict = dict(zip(list(ques_tmp_df['ques']), list(ques_tmp_df['area'])))
    ...
    with open('train_test/%s_area.txt' % train_or_test, 'w', encoding='utf-8') as w:
